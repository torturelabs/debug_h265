...
//        " 0, 0, 0, 1",
    var fragmentShaderSource = [
        ...
        "(",
        " 1.1643828125, 1.59602734375, 0, -.87078515625,",       // YCbCr2R
        " 1.1643828125, -.81296875, -.39176171875, .52959375,",  // YCbCr2G
        " 1.1643828125, 0, 2.017234375, -1.081390625,",          // YCbCr2B
        " 0, 0, 0, 1",
        ");",
        "void main(void) {",
...
WebGLPlayer.prototype.renderFrame2 = function (width, height) {
    if (!this.gl) {
        console.log("[ER] Render frame failed due to WebGL not supported.");
        return;
    }

  let buf = Module._get_mega_buffer_y()
  if (buf == 0) return;

  // VM4616:1 Uncaught RangeError: Invalid typed array length: 1036800
  let YFrame = new Uint8Array(Module.HEAPU8.buffer, Module._get_mega_buffer_y(), width*height)
  try {
    let UFrame = new Uint8Array(Module.HEAPU8.buffer, Module._get_mega_buffer_u(), width*height / 2)
    let VFrame = new Uint8Array(Module.HEAPU8.buffer, Module._get_mega_buffer_v(), width*height / 2)
    ...
    gl.y.fill(width, height, YFrame);
  gl.u.fill(width >> 1 , height >> 1, UFrame);
  gl.v.fill(width >> 1, height >> 1, VFrame);
